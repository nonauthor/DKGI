...
def readtsv(data_name, tsv_file, e_r_2id ,vector):
    if data_name == './data/classification/mutag/':
        nodes_header = 'bond'
        label_header = 'label_mutagenic'
    elif data_name == './data/classification/am/':
        nodes_header = 'proxy'
        label_header = 'label_cateogory'
    else: print('Something wrong')

    str2vec = np.ones((tsv_file.index.size,vector.shape[1]))
    label = tsv_file[label_header].values

    for i,x in enumerate(tsv_file[nodes_header].values):
        id = e_r_2id[x]
        vec = vector[id]
        str2vec[i] = vec

    return str2vec, label

def readtsv2id(data_name, tsv_file, e_r_2id):
    if data_name == './data/classification/mutag/':
        nodes_header = 'bond'
        label_header = 'label_mutagenic'
    elif data_name == './data/classification/am/':
    ...
    for i,x in enumerate(tsv_file[nodes_header].values):
        id = e_r_2id[x]
        x_id.append(id)
    return torch.LongTensor(x_id), label
...
def label2onehot(label):
    ...
    label_onehot = torch.zeros((label.size, len(label_set)))
    for i,lb in enumerate(label):
        index = label_set.index(lb)
        label_onehot[i][index] = 1
    return label_onehot
